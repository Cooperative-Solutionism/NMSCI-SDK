...
describe('NmsciSdk', () => {
  ...
  it('exposes grouped message collection root helpers', async () => {
    ...
    await sdk.flowNodeRegister.list({ flowNodePubkey: flowPubkey, page: 1, size: 10 });
    await sdk.centralPubkeyEmpower.list({ flowNodePubkey: consumePubkey, page: 2, size: 20 });
    await sdk.flowNodeLocked.list({ page: 3, size: 30 });
    await sdk.centralPubkeyLocked.list({ page: 4, size: 40 });

    expect(requested).toEqual([
      `https://example.test/flow-node-registrations?flowNodePubkey=${flowPubkey}&page=1&size=10`,
      `https://example.test/central-pubkey-empowerments?flowNodePubkey=${consumePubkey}&page=2&size=20`,
